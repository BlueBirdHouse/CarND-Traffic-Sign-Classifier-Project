...
Data_Length = 12630
X_test_Matlab = np.zeros([Data_Length,32,32,3],dtype = np.uint8)
y_test_Matlab = np.zeros([Data_Length],dtype = np.uint8)
...
for Counter in range(Data_Length):
    ...
    X_test_Matlab[Counter] = AFig
    y_test_Matlab[Counter] = Alabel
...
if not os.path.isfile(pickle_file):
    ...
    try:
        with open(pickle_file, 'wb') as pfile:
            pickle.dump(
                    {
                        'X_test_Matlab': X_test_Matlab,
                        'y_test_Matlab': y_test_Matlab
                    },
